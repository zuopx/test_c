...
public:
  ...
  Base(std::string s, int v) : Base(s) { value = v; }

  // final constructor
  virtual void foo() final { return; }
  virtual void foo(int v) { value = v; }
};
...
public:
  ...
  // inherit constructor
  Subclass(double f, int v, std::string s) : Base(s, v) { floating = f; }
